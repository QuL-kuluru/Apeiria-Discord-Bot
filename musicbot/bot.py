...
class MusicBot(discord.Client):
    def __init__(self, config_file=None, perms_file=None):
        ...
        if not self.autoplaylist:
            log.warning("オートプレイリストがありません、アプリを終了します")
            self.config.auto_playlist = False
        else:
            log.info("プレイリスト読み込み... {} 件".format(len(self.autoplaylist)))

        if self.blacklist:
            log.debug("ブラックリスト読み込み... {} 件".format(len(self.blacklist)))
    ...
    def owner_only(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if not orig_msg or orig_msg.author.id == self.config.owner_id:
                ...
                raise exceptions.PermissionsError("このコマンドはオーナー以外の命令は受け付けません。", expire_in=30)
    ...
    def dev_only(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if orig_msg.author.id in self.config.dev_ids:
                ...
                raise exceptions.PermissionsError("アペイリアシステムへの要求は拒否されました。", expire_in=30)
    ...
    async def _join_startup_channels(self, channels, *, autosummon=True):
        ...
        def _autopause(player):
            if self._check_if_empty(player.voice_client.channel):
                log.info("空のチャンネルで最初の自動一時停止")

                player.pause()
                self.server_specific_data[player.voice_client.channel.server]['自動一時停止'] = True
        ...
        for server in self.servers:
            ...
            if server.me.voice_channel:
                log.info("前回地点から再生可能なチャンネル {0.server.name}/{0.name}".format(server.me.voice_channel))
                channel_map[server] = server.me.voice_channel

            if autosummon:
                owner = self._get_owner(server=server, voice=True)
                if owner:
                    log.info("オーナーを発見 \"{}\"".format(owner.voice_channel.name))
                    channel_map[server] = owner.voice_channel

        for server, channel in channel_map.items():
            if server in joined_servers:
                log.info("既に次のチャンネルにいます \"{}\", skipping".format(server.name))
                continue

            if channel and channel.type == discord.ChannelType.voice:
                log.info("参加中 {0.server.name}/{0.name}".format(channel))

                chperms = channel.permissions_for(server.me)

                if not chperms.connect:
                    log.info("参加できません \"{}\", 権限がありません.".format(channel.name))
                    continue

                elif not chperms.speak:
                    log.info("参加しません \"{}\", 話す権限がありません.".format(channel.name))
                    continue
                ...
                try:
                    ...
                    log.info("入室 {0.server.name}/{0.name}".format(channel))
                    ...
                    log.debug("参加に失敗しました {0.server.name}/{0.name}".format(channel), exc_info=True)
                    log.error("参加に失敗しました {0.server.name}/{0.name}".format(channel))

            elif channel:
                log.warning("参加していません {0.server.name}/{0.name}, that's a text channel.".format(channel))
    ...
    async def get_voice_client(self, channel: discord.Channel):
        ...
        async with self.aiolocks[_func_() + ':' + channel.server.id]:
            ...
            log.debug("接続しました {:0.1f}s".format(t1-t0))
            log.info("接続しました {}/{}".format(channel.server, channel))
            ...
            if server.id not in self.players:
                if not create:
                    raise exceptions.CommandError(
                        'ネガティブ。アペイリアはまだ音声チャンネルに参加していません  '
                        '呼び出すには %ssummon を使用してアペイリアを呼び出してください' % self.config.command_prefix)
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if self.config.now_playing_mentions:
                newmsg = '%s - の **%s** は %sチャンネルで再生中です' % (
                    entry.meta['author'].mention, entry.title, player.voice_client.channel.name)
            else:
                newmsg = '再生中 %s チャンネル **%s**' % (
...
#######################################################################################################################
    ...
    async def on_ready(self):
        ...
        log.info('接続しました  Apeiria v{}\n'.format(BOTVERSION))

        self.init_ok = True

        ################################

        log.info("ボットID:   {0}/{1}#{2}{3}".format(
            self.user.id,
            self.user.name,
        ...
        if self.config.bound_channels:
            ...
            if chlist:
                ...
                print("任意のデフォルトテキストチャンネルを指定していません")

            if invalids and self.config.debug_mode:
                print(flush=True)
                log.info("任意のデフォルト音声チャンネルを指定していません:")
        ...
        if self.config.autojoin_channels:
            ...
            if chlist:
                ...
                log.info("自動参加を有効にしていません")

            if invalids and self.config.debug_mode:
                print(flush=True)
                log.info("自動的にテキストチャンネルに参加できませんでした:")
                [log.info(' - {}/{}'.format(ch.server.name.strip(), ch.name.strip())) for ch in invalids if ch]
        ...
        log.info("  デフォルト音量: {}%".format(int(self.config.default_volume * 100)))
        log.info("  スキップ条件: {} 票 または {}%".format(
        ...
        if self.config.delete_messages:
            log.info("    呼び出し削除: " + ['無効', '有効'][self.config.delete_invoking])
        log.info("  デバッグモード: " + ['無効', '有効'][self.config.debug_mode])
        log.info("  ダウンロードキャッシュ " + ['削除', '保存'][self.config.save_videos])
        print(flush=True)
    ...
    async def cmd_help(self, command=None):
        ...
        if command:
            ...
            helpmsg = "**ラージャ。現在使用できるコマンドを表示します。**\n```"
            commands = []
    ...
    async def cmd_blacklist(self, message, user_mentions, option, something):
        ...
        ブラックリストにユーザーを追加する/リストから削除することができます。
        ブラックリストに載ったユーザーはbotを使用することができません。
        """
    ...
    async def cmd_id(self, author, user_mentions):
        ...
        """
        if not user_mentions:
            return Response('あなたのIDは `%s`' % author.id, reply=True, delete_after=35)
        else:
            usr = user_mentions[0]
            return Response("%sのIDは `%s`" % (usr.name, usr.id), reply=True, delete_after=35)
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        ネガティブ。URLの指定がありません。YouTube、またはSoundCloudのURLを指定してください。
        """
        ...
        if permissions.max_songs and player.playlist.count_for_user(author) >= permissions.max_songs:
            raise exceptions.PermissionsError(
                "あなたのキューできる最大数を超えました。あなたのキューできる最大数は (%s)" % permissions.max_songs, expire_in=30
            )
        ...
        if 'entries' in info:
            ...
            if not permissions.allow_playlists and ':search' in info['extractor'] and len(info['entries']) > 1:
                raise exceptions.PermissionsError("ネガティブ。あなたはプレイリストの編集を許可されていません", expire_in=30)
            ...
            reply_text = "キューに追加されました **%s** はプレイされます キュー内の順番: %s"
    ...
    async def _cmd_play_playlist_async(self, player, channel, author, permissions, playlist_url, extractor_type):
        ...
        if not info:
            raise exceptions.CommandError("このプレイリストは再生できません")

        num_songs = sum(1 for _ in info['entries'])
        t0 = time.time()

        busymsg = await self.safe_send_message(
            channel, "%s 曲を処理中" % num_songs)  # TODO: From playlist_title
        await self.send_typing(channel)
        ...
        if extractor_type == 'youtube:playlist':
            try:
                ...
                log.error("プレイリスト処理にエラーが発生しました", exc_info=True)
    ...
    async def cmd_search(self, player, channel, author, permissions, leftover_args):
        """
        使い方:
            {command_prefix}search [サービス] [番号] クエリ

        サービスを検索してキューに追加します。
        - サービス: 次のいずれかのサービスを指定すること:
            - youtube (yt) (指定しなかった場合)
            - soundcloud (sc)
            - yahoo (yh)
        - 番号: この番号で検索数を指定し、ユーザーが曲を選択することができます
          - デフォルトでは1曲指定
          - 補足: 検索クエリが数字で始まる場合、
                  クエリを引用符で囲む必要があります
            - 例: {command_prefix}search 2 "Apeiria"
        """
        ...
        search_msg = await self.send_message(channel, "データベースを検索中です")
        await self.send_typing(channel)
        ...
        if not info:
            return Response("ネガティブ。データは見つかりませんでした", delete_after=30)
        ...
        for e in info['entries']:
            ...
            confirm_message = await self.safe_send_message(channel, "オーナー、これでいいですか？  `y` か `n` もしくは `exit`を入力してください")
            response_message = await self.wait_for_message(30, author=author, channel=channel, check=check)

            if not response_message:
                await self.safe_delete_message(result_message)
                await self.safe_delete_message(confirm_message)
                return Response("わかりました、気にしないでください", delete_after=30)
            ...
            if response_message.content.lower().startswith('y'):
                ...
                return Response("ラージャ。プレイリストに追加します", delete_after=30)
            else:
                await self.safe_delete_message(result_message)
                await self.safe_delete_message(confirm_message)
                await self.safe_delete_message(response_message)

        return Response("ごめんなさい、これ以上のデータがみつかりませんでした \N{SLIGHTLY FROWNING FACE}", delete_after=30)
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            ...
            action_text = 'ストリーム中' if streaming else '再生中'

            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                np_text = "現在 {action}: **{title}** 次のユーザーによって追加されました **{author}** {progress}\n\N{WHITE RIGHT POINTING BACKHAND INDEX} <{url}>".format(
                    action=action_text,
                ...
                np_text = "現在 {action}: **{title}** {progress}\n\N{WHITE RIGHT POINTING BACKHAND INDEX} <{url}>".format(
                    action=action_text,
                    title=player.current_entry.title,
                    progress=prog_str,
                    url=player.current_entry.url
    ...
    async def cmd_summon(self, channel, server, author, voice_channel):
        ...
        if not author.voice_channel:
            raise exceptions.CommandError('ネガティブ。オーナーは音声チャンネルにいません')
        ...
        log.info("参加中 {0.server.name}/{0.name}".format(author.voice_channel))
    ...
    async def cmd_pause(self, player):
        ...
        if player.is_playing:
            ...
            raise exceptions.CommandError('ネガティブ、アペイリアは曲を再生していません。', expire_in=30)
    ...
    async def cmd_resume(self, player):
        ...
        if player.is_paused:
            ...
            raise exceptions.CommandError('ネガティブ、アペイリアは再生中です。', expire_in=30)
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel):
        ...
        曲中、現在いるメンバーで投票を行い、スキップ多数の場合スキップされます。オーナーの命令があれば即時スキップします。
        """

        if player.is_stopped:
            raise exceptions.CommandError("ネガティブ。 プレイヤーが再生されていないため、スキップできません", expire_in=20)
    ...
    async def cmd_volume(self, message, player, new_volume=None):
        ...
        if not new_volume:
            return Response('現在の音量は `%s%%`' % int(player.volume * 100), reply=True, delete_after=20)
        ...
        try:
            ...
            raise exceptions.CommandError('ネガティブ。{} は誤った入力です'.format(new_volume), expire_in=20)
        ...
        if 0 < new_volume <= 100:
            ...
            return Response('ラージャ。音量変更 %d％ から %d％' % (old_volume, new_volume), reply=True, delete_after=20)
    ...
    async def cmd_queue(self, channel, player):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                lines.append("再生中: **%s**  **%s** により追加されました %s\n" % (
    ...
    async def cmd_clean(self, message, channel, server, author, search_range=50):
        ...
        履歴を [range] 通のアペイリアからのメッセージを削除します。　指定しない場合 50通、 最大 1000通のメッセージを削除します。
        """
        ...
        if self.user.bot:
            if channel.permissions_for(server.me).manage_messages:
                ...
                return Response('ラージャ。 {} 件のメッセージを削除しました。{}'.format(len(deleted), '　' * bool(deleted)), delete_after=15)
    ...
    async def cmd_setnick(self, server, channel, leftover_args, nick):
        ...
        if not channel.permissions_for(server.me).change_nickname:
            raise exceptions.CommandError("ネガティブ、アクセスを拒否されました。：アペイリアはこのネットワークにアクセスする権限がありません")
    ...
    async def on_voice_state_update(self, before, after):
        ...
        if state.joining or state.leaving:
            log.info("{0.id}/{0!s} は {1} {2}/{3}".format(
                state.member,
                '参加しました' if state.joining else '退室しました',
                state.server,
        ...
        autopause_msg = "{state} しました {channel.server.name}/{channel.name} {reason}"
    ...
    async def on_server_update(self, before:discord.Server, after:discord.Server):
        if before.region != after.region:
            log.warning("サーバー \"%s\" リージョンチェンジ: %s -> %s" % (after.name, before.region, after.region))
